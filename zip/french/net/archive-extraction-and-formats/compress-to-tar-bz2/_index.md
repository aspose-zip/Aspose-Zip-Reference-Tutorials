...
Avant de commencer, vérifiez‑nous que vous avez tout ce dont vous avez besoin.
...
- **Puis‑je compresser plusieurs fichiers ?** Oui – ajoute autant d'entrées que vous le souhaitez à l'archive Tar
- **Est‑elle compatible avec .NET 6+ ?** Absolument, Aspose.Zip prend en charge .NET Framework et .NET Core/5/6

## Comment compresser tar
...
## Pourquoi compresser des fichiers dans TarBz2 avec Aspose.Zip ?
...
- **Contrôle fin** – Choisissez les fichiers à inclure, définis des noms d'entrée personnalisés et diffusez directement sur le disque.
...
- **Document Directory** – Un dossier contenant les fichiers que vous souhaitez archiver. Dans les exemples nous les référencions avec la variable `dataDir`.
...
## Étape 2 : Ajouter les fichiers à l’archive tar et créer une archive TarBz2
...
|--------------|--------|--------------|
| **Erreur fichier introuvable** | Chemin `dataDir` incorrect ou extension de fichier manquant | Vérifiez le chemin complet et assurez-vous que le fichier existe. |
| **Archiver la vidéo** | Aucune entrée ajoutée avant `bz2.Save` | Ajoutez au moins un appel à `CreateEntry`. |
